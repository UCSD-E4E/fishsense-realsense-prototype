        ...
        for i in range(len(fishes)):
            ...
            for x in range(xmin, xmax):
                for y in range(ymin, ymax):
                    ...
                    depth_val = depth_frame.get_distance(int(x), int(y))
                    depth_list.append(depth_val)
                    depth_dic[depth_val] = (x, y, depth_val, depth_val)

            median = statistics.median(depth_list)
            ...
            #         point1[2] - point2[2], 2))
            # cv2.putText(result, "{} cm".format(dist*100), (fishes[i][2], fishes[i][1]), color=(0,0,0),thickness=2, fontFace=1, fontScale=2)
